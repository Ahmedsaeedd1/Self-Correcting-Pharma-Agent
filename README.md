...
Dense Semantic Retrieval: Safe queries are vectorized using native Gemini embeddings and pushed against a Pinecone Vector Database containing thousands of chunked PDFs covering clinical trial protocols and regulatory data.
Self-Reflective Grading: A discrete LLM grader objectively analyzes the retrieved context against the original query. It asks a binary question: "Does this internal documentation actually answer the user's question?"
Dynamic Web Fallback: If the grader determines the internal vector context is insufficient (e.g., the user asks about an FDA commissioner appointed after our documents were published), the agent dynamically pivots to execute a live internet search via the Tavily API.
Final Synthesis: Only after the context has been validated—or supplemented by the web—is the data passed to the final generative node to formulate a concise, domain-expert response.
Security Features
...
1. Requirements
Python 3.12+
A Google Gemini API Key (gemini-2.5-flash and gemini-embedding-001)
A Pinecone Vector DB API Key
